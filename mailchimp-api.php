...
function response($responseStatus, $responseMsg) {
	$result = json_encode(array('responseStatus' => $responseStatus, 'responseMsg' => $responseMsg));
	die($result);
}
...
$api = new MCAPI($option[$key[0]]['api_key']);
$listId = $api->lists(array('list_name' => $option[$key[0]]['list_name']), 0, 1);
$listId = @$listId['data'][0]['id'];
if (!$listId) response('err', 'listid');
$emailAddress = trim(strtolower($_POST['newsletter-email']));
$retval = $api->listSubscribe($listId, $emailAddress, array(), null, true, null, null, true);
if ($api->errorCode) {
